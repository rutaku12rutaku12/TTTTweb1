...
public class FileService {
    ...
    // 방법2) 프로젝트내 build : 서버실행했을떄 컴파일된 코드 즉] 서버(24시간)의 갖는 실행된 폴더

    // 1. 현재 프로젝트의 최상위 디렉토리(폴더) 경로 찾기
    private String baseDir = System.getProperty("user.dir");
    // 2. 방법2 처럼 개발자폴더가 아닌 실행된 서버의 폴더로 업로드경로 지정하기 , *개발환경* 에 따라 달라진다.
    ...
    public String fileUpload( MultipartFile multipartFile ){
        // 1. 업로드한 파일명이 중복일때, 다른 파일이지만 파일명은 같을 수 있다.
        ...
        String uuid = UUID.randomUUID().toString();
            ...
            // 이유 : uuid 와 파일명 구분을_언더바 사용하기 위한 내부적인 규칙

        String fileName = uuid + "_" + multipartFile.getOriginalFilename().replaceAll("_","-"); // _를 -로 바꾼다.
        // 2. 업로드 경로 와 파일명 합치기
        ...
        if( ! pathFile.exists() ){ // .exists() : 지정한 겨로가 존재하면 true 아니면 false
            pathFile.mkdir(); // .mkdir() : 지정한 경로 생성 메소드
        }
        // 4. 업로드 한 파일의 경로를 file 클래스 생성
        File uploadFile = new File( uploadFilePath );
        // 5. 업로드( 파일/바이트 이동) 하기, .transferTo( file객체 ) : 지정한 file객체의 경로로 업로드 파일을 이동
        try { multipartFile.transferTo(uploadFile); // multipartFile(업로드된파일) , 일반예외 발생
    ...
    public void fileDownload(String fileName , HttpServletResponse response ){
        ...
        // 3. 업로드할 파일을 자바(바이트)로 가져오기(파일읽기)
            ...
            fin.close(); // 필수는 아니지만 대용량에서는 안전하게 입력스트림 객체 를 직접 닫기 권장!
        ...
        // 4. 업로드할 파일을 사용자에게 (response 객체 이용한) 응답하기.
            ...
            fout.write( bytes );
            // 4-3 안전하게 스트림 닫기
            fout.close(); // 필수는 아니지만 대용량에서는 안전하게 출력스트림 객체 를 직접 닫기 권장!
