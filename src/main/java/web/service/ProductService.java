...
public class ProductService {
    ...
    public List<ProductDto> getAllProduct(){
        ...
        // 2. 모든 제품의 이미지 조회
        for( ProductDto productDto : productDtoList ){ // 조회된 모든 제품 dto 를 반복
            List<String> images =
            productDao.getProductImages(productDto.getPno()); // 특정한 제품의 pno 를 이용하여 모든 제품 이미지 조회
            productDto.setImages(images); // 조회된 모든 이미지명을 특정한 제품의 dto 포함
        }
        // 3. 반환
   ...
   // [3-1] 개별 제품 조회
    public ProductDto getProduct( int pno ){
        ...
        if( productDto != null ){
            List<String> images = productDao.getProductImages( productDto.getPno() ); // [2-2] 메소드 호출
