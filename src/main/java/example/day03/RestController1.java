...
// 어노테이션이란? (라벨) 택배상자안에 있는 실질적인 내용물이 아닌, 택배상자 옆에 붙인 '유리 취급주의' 부착
...
public class RestController1 {
    ...
    // Whitelabe Error Page : 요청은 정상이지만 , 응답은 없다.
    ...
    public boolean method2( @RequestParam( name ="name" , defaultValue = "홍길동" , required = true ) String name ) {
        ...
        // + @RequestParam( name ="URL 매개변수명" , defaultValue = "초기값" , required = true/false );
        // ++ name = "URL 매개변수명" : 주소(URL) 상의 ? 뒤로 매핑(연결) 할 매개변수명 명시 , 단 자바 매겨변수와 이름이 동일시 생략가능
    ...
    // http://localhost:8080/day03/param2?name=유재석&age=40
    ...
    public String method4( @RequestParam Map<String , String > map ){
        ...
        // Map : KEY 와 VALUE 구성해서 한쌍(ENTRY) 구성하여 여러개 ENTRY 저장 구조
    ...
    * 2) BODY(본문)이란 ? HTTP 본문에 매개변수 표현 , 매개변수 노출X , 보안 안전 , POST/PUT 에서 JSON 타입으로 객체 지원
    * +++ --> HTTPS

                    @RequestPram                        vs       @ModelAttribute                               vs       @RequestBody
    역할            단일 파라미터에 변수 바인딩(저장)                복수 파라미터에 객체 바인딩(저장)                          본문(body) 객체 바인딩(저장)
    생략기준        변수명이 일치할경우(쿼리스트링의매개변수)          DTO 타입일때 기본적용(+생성자,+Setter)                    DTO
    주요타입        기본형:int/String/List/Map                     DTO(개발자가만든객체)                                    DTO
    처리가능요청     쿼리스트림<form:첨부파일 x>                      쿼리스트링,<form:첨부파일 o>                             본문(body)
