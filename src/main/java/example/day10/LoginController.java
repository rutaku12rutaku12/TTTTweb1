...
public class LoginController {
    ...
    // 1. 로그인 : 아이디와 패스워드가 일치하면 *세션* 에 로그인 성공항 회원번호 저장
    ...
    public boolean login(@RequestBody LoginDto loginDto , HttpServletRequest request ){
        ...
        System.out.println( request.getRemoteAddr()); // 통신을 요청한 클리이언트(사용자)의 IP 확인
        System.out.println( request.getRemotePort()); // 통신을 요청한 클리이언트(사용자)의 PORT 확인
        System.out.println( request.getHeader("User-Agent")); // 통신을 요창한 클라이언트(사용자)의 브라우저정보 확인, HTTP Header(통신부가정보)
        // **** 세션 정보 가져오기 , 세션이란? 톰캣 서버내 저장소 ( key-value /MAP 컬렉션 사용 )
        HttpSession session = request.getSession();
        System.out.println( session.getId() ); // 세션의 식별 번호 : 브라우저 마다 , pc 마다 다르게 할당 된다. // 사용자 마다의 개인 저장소
        System.out.println( session.getCreationTime() ); // 세션의 생성시간(ms) : 최초 요청한 처리
        ...
        session.setAttribute("loginMno",3); // .setAttribute( "속성명" , value );
        // loginMon 라는 속성명 으로 3이라는 데이터 저장 하겠다는 뜻
        return true;
    }
    ...
    public boolean info( HttpServletRequest request ){
        // 1. 요청 서블릿에서 세션정보 꺼내기
        HttpSession session = request.getSession();
        // 2. 세션정보내 원하는 속성명 값 꺼내기 개발자의 의도와 상관없이 값은 무조건 object 타입
