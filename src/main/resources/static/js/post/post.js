...
const viewPageButtons = async( data )=>{
    ...
    // ************************* 이전버튼 , 유효성 검사 : 만약에 현재페이지가 1 이하면 1고정  ************************
    html += `<li> <a href="post.jsp?cno=${cno}&page=${currentPage <= 1 ? 1 : currentPage-1}${searchURL}"> 이전 </a></li>`
    // ************************* 페이지버튼 ************************
    for( let i = startBtn ; i <= endBtn ; i ++ ){
        html += `<li> <a href="post.jsp?cno=${cno}&page=${i}${searchURL}" 
        style ="${ i == currentPage ? 'color : red' :''}">
        ${i}
        </a></li>`

    }
    // ************************* 다음버튼 , 유효성 검사 만약에 다음페이지가 전체페이지수보다 커지면 전체페이수로 고정 ************************
